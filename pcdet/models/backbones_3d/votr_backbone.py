...
from .LA_attention import LinAngularAttention
...
class Attention3d(nn.Module):
    def __init__(self, input_channels, output_channels, ff_channels, dropout, num_heads, attention_modes):
        ...
        '''
        self.mhead_attention = LinAngularAttention(input_channels, num_heads=num_heads, proj_drop=dropout, sparse_reg=self.train)
        # self.mamba = mamba_ssm.Mamba(d_model=input_channels,d_state=16,d_conv=4,expand=2)
        # config = MambaConfig(d_model=input_channels, n_layers=1)
...
class SparseAttention3d(Attention3d):
    def __init__(self, input_channels, output_channels, ff_channels, dropout, num_heads, attention_modes, strides, num_ds_voxels,
        ...
        # config = MambaConfig(d_model=input_channels, n_layers=1)
        # self.mamba  = Mamba(config)
        # self.rmsnor = RMSNorm(input_channels)
    ...
    def forward(self, sp_tensor):
        ...
        '''
        attend_features = self.mhead_attention(query_features, key_features, key_features)
        # attend_features = self.mamba(query_features)
        
        # print(torch.max(attend_features))
        # print(torch.min(attend_features))
        attend_features = self.drop_out(attend_features)
        # attend_features = self.rmsnor(attend_features)
    ...
    def forward(self, batch_dict):
        ...
        for attention_block in self.backbone:
            ...
            x_convs.append(SparseConvTensor(
                ...
                indices=sp_tensor.indices.clone(),
            ))
